...
That gate was adversarially audited on its own, hunting specifically for a way a held record could reach the public page. No live exploit was found; the file's existing defense in depth is real, and the full compiled HTML is re-scanned for a held identifier as the last step before every write, catching anything upstream code might do wrong. One structural gap was real but not exploitable today: a record_id could exist in both `records/` and `records/held/` with nothing checking for the collision. Traced through by hand and confirmed the build already refused in that exact case, one step later than it should have and by coincidence rather than by design: the public copy's own id happens to be printed on the page, which is what the final rescan actually catches. `load_all()` now refuses the collision directly, at its source, rather than relying on the colliding id happening to be visible page text. Said plainly: this closes a coincidence, not a leak that was reachable, and the difference matters enough to state rather than let the fix read as more urgent than it was.

**Built, the HTTP surface:** `gateway/serve.py`, standard library only. `POST /v1/decisions`, `POST /v1/live-check`, `POST /v1/receipts/verify`, `GET /healthz`. Observe is the default and `--enforce` is required to block; the server refuses to start on a policy it cannot evaluate rather than running with a permissive gap. Every error path, including an unhandled exception in its own handler, returns an explicit non-permit rather than anything a caller could read as approval. That property is the one most heavily covered by its tests: mutating the failure shape to a permit fails 25 of them.

`/v1/live-check` is where the HTTP surface and `gateway/live_adapter.py` meet: `{"ticker": "AAPL"}` fetches that ticker's real daily series from the same authority the suite uses, runs the real classifiers against it, and judges the result under whichever policy the running gateway was started with, not a policy fixed in the CLI script. A caller can supply its own `subject` (the series its own tool produced) to have that judged instead of the authority's own faithful echo, and a `context` object for policies that gate on more than notional. The authority not answering is `502` with `AUTHORITY_UNAVAILABLE`, not a permit and not a crash: there is no reference to compare against, so there is no verdict, not even by default. Two mutations were run against this path: making the authority-unavailable branch answer 200 instead of failing closed, and dropping the caller's context so a real policy's limits go unevaluated. Both are caught.

The authority fetch is cached per ticker for 30 seconds (`healthz` reports the TTL), found by hitting it: testing this endpoint a handful of times in a row was enough to get Yahoo's own rate limit back, which would otherwise show up as the gateway itself looking unreliable. A failed fetch is never cached as an answer, so an authority that comes back does not stay reported down. Two more mutations were run: dropping the cache write entirely, and caching a failed fetch as if it were a real one. Both are caught.
